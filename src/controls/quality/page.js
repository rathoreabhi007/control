...
const createInitialNodes = () => [
    // All nodes arranged in a straight horizontal line
    {
    ...
    },
    {
        ...
        type: 'custom',
        data: { fullName: 'QA Rules & PreProcess', status: 'idle' },
    ...
    },
    {
        id: 'apply_rules_comp',
        type: 'custom',
        data: { fullName: 'Output Generation', status: 'idle' },
...
export default function QualityControl({ instanceId }) {
                    ...
                    <div key={key} className="flex flex-col">
                        ...
                        {key === 'runEnv' ? (
                            ...
                            >
                                ...
                                <option value="production" className="text-gray-800">Production</option>
                            </select>
                        ) : key === 'testRun' ? (
                            <select
                                value={value || 'off'}
    ...
    useEffect(() => {
        const checkHealth = async () => {
            try {
                // console.log('🔍 Quality: Performing initial backend health check...');
                ...
                // console.log('✅ Quality: Initial health check completed:', isHealthy ? 'Healthy' : 'Unhealthy');
            } catch (error) {
                console.error('Quality: Initial health check failed:', error.message);
                ...
                return new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => {
                        ...
                        if (!wsStatus.isConnected && !isResolved) {
                            console.warn(`WebSocket not connected, falling back to polling for ${nodeId}`);
                            WebSocketService.unsubscribe(processId);
    ...
    }, [nodes, setNodes, updateNodeStatus, updateNodeOutput, dependencyMap, validatedParams, tempFileBasePrefix, instanceId]);
