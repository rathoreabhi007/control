...
/**
 * ETL Log Viewer Component - Similar to control-runs log viewer but for ETL tasks
 */
const ETLLogViewer = ({ taskId, nodeId, onClose, taskStatus = null }) => {
    ...
    const loadLogs = useCallback(async (incremental = false) => {
        ...
        try {
            ...
            // Path: /api/etl/logs/{task_id}?stream=true
            const logUrl = `${API_BASE_URL}/api/etl/logs/${taskId}?stream=true`;
    ...
    }, [taskId, lastLineCount]);

    // Initial load - reset when task changes
    useEffect(() => {
        setLastLineCount(0); // Reset line count when task changes
        setLogs(''); // Clear logs
    ...
    const downloadLogs = () => {
        ...
        a.href = url;
        a.download = `${taskId}_${nodeId || 'etl'}_logs.txt`;
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <p style={{ margin: '4px 0 0 0', color: '#666', fontSize: '13px' }}>
                            {nodeId && `Node: ${nodeId} • `}Task ID: {taskId}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
